...
const Queue = () => {
  ...
  const [entriesExist, setEntriesExist] = useState(false);
  const [authDeletionError, setAuthDeletionError] = useState(false);
  const [popUpIsOpen, setPopupIsOpen] = useState(false);
  const [popUpDeleteError, setPopUpDeleteError] = useState(false);
  ...
  function openPopup(event) {
    ...
    setCurrentItemName(event.target.parentElement.parentElement.firstChild.innerHTML)
    setPopupDeleteSuccess(false)
  }

  function confirmPopup() {
    return deleteQueueItem(currentItem);
  }

  function cancelPopup() {
    setPopupIsOpen(false)
    setPopUpDeleteError(true)
    setCurrentItem('')
    setCurrentItemName('')
    return;
  }
  ...
  function deleteQueueItem(event) {
    setAuthDeletionError(false);
    setPopupIsOpen(false);
    setPopUpDeleteError(false);
    ...
    apiClient.deleteQueueItem({ 'queueID': queueID }).then((data) => {
      apiClient.removeFromUserHistory(data).then((res) => {
        apiClient.deletePrediction(res.data).then((res) => {
          apiClient.deleteFile(res.data).then((data) => {
    ...
    }).catch((err) => { setAuthDeletionError(true) })
  }
  ...
  function createItems(item) {
    ...
    return (
      <tr key={item._id.$oid} id={item._id.$oid}>
        ...
        <td> {timeRunning} </td>
        <td> <span className={'remove-file ' + (isOwner ? '' : 'hidden')} onClick={openPopup()}></span> </td>
      </tr>
    )
  }
  ...
  if (!isFetchingData) {
    ...
    var listItems = queueEntries.map(createItems);
  }
  return (
    <div className="container">
      ...
      <div className="container-fluid">
        ...
        <p className={'login-error ' + (authDeletionError ? '' : 'hidden')}>
          {t('queue.authdeletionerror')}
        </p>

        <p className={'login-error ' + (popUpDeleteError ? '' : 'hidden')}>
          {t('queue.popupdeleteerror')}
        </p>

        <p className={'delete-success ' + (popupDeleteSuccess ? '' : 'hidden')}>
          {t('queue.popupdeletesuccess')}
        </p>

        <Table striped bordered hover className={"queue-table " + (entriesExist ? '' : 'hidden')}>
          <thead>
            <tr>
        ...
        <p className={"queue-table " + (entriesExist ? 'hidden' : '')}>{t('queue.queueempty')}</p>
      </div>
    </div>
  )
}
